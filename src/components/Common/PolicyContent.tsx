...
const POLICY_DATA = [
  {
    title: "제1조 (목적)",
    content: "본 약관은 'POSiT!' 서비스(이하 '서비스')가 제공하는 모든 기능 및 콘텐츠를 이용함에 있어, 서비스와 이용자 간의 권리, 의무 및 책임 사항을 규정함을 목적으로 합니다."
  },
  {
    title: "제2조 (용어의 정의)",
    content: [
  ...
  },
  {
    ...
    content: "서비스는 이용자의 개인정보를 보호하기 위해 '개인정보 처리방침'을 수립하고 이를 준수합니다. 이용자의 명시적인 동의 없이 제3자에게 개인정보를 제공하지 않습니다."
  },
  {
...
export default function PolicyContent({ bottomBar }: PolicyContentProps) {
        ...
        <div className="flex flex-col gap-[40px] pb-[182px]">
          {POLICY_DATA.map((item, index) => (
            <section key={index} className="flex flex-col">
              <h2 className="text-[#5D5D5D] typo-16-regular mb-[2px]">{item.title}</h2>
              
              {Array.isArray(item.content) ? (
                <div className="flex flex-col gap-[2px]">
                  {item.content.map((line, i) => (
                    <div key={i} className="flex gap-[8px] text-[#5D5D5D] typo-16-regular">
                ...
                <p className="text-[#5D5D5D] typo-16-regular">
                  {item.content}
                </p>
              )}
            </section>
